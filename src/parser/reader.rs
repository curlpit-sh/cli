...
async fn parse_request_contents(
    ...
    apply_default_headers(&mut headers, &environment.default_headers);
    ...
    if let Some((_, raw_line)) = lines.peek() {
        ...
        if let Some(rest) = trimmed.strip_prefix("@body") {
            ...
            if !raw_body.trim().is_empty() {
                ...
                body_bytes = Some(expanded_body.len());
...
fn apply_default_headers(headers: &mut Vec<(String, String)>, defaults: &HashMap<String, String>) {
...
mod tests {
    ...
    #[tokio::test]
    async fn parse_request_file_applies_default_headers() -> Result<()> {
        let temp = tempdir()?;
        let base = temp.path();
        let mut env_context = base_environment(base);
        env_context
            .default_headers
            .insert("X-Default".to_string(), "default".to_string());
        env_context
            .default_headers
            .insert("X-Override".to_string(), "default".to_string());

        let request_path = base.join("default.curl");
        tokio::fs::write(
            &request_path,
        ...
        assert!(parsed
            ...
            .headers
            .iter()
            .any(|(name, value)| name == "X-Default" && value == "default"));
        assert!(parsed
            .request
            .headers
            .iter()
            .any(|(name, value)| name == "X-Override" && value == "custom"));
        Ok(())
    }
