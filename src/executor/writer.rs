...
pub(super) fn write_response_body(
    ...
    let body_to_write = prepare_body_for_storage(bytes, content_type, extension);
    ...
    if let Some(base_dir) = response_dir {
        ...
        fs::write(&path, body_to_write.as_ref())
...
fn prepare_body_for_storage<'a>(
    bytes: &'a [u8],
    content_type: Option<&str>,
    extension: &str,
) -> Cow<'a, [u8]> {
    if !should_format_json(content_type, extension) {
    ...
    // Avoid prettifying empty payloads
...
fn should_format_json(content_type: Option<&str>, extension: &str) -> bool {
...
mod tests {
    ...
    fn write_response_body_creates_files_under_response_dir() -> Result<()> {
        ...
        let first_contents = std::fs::read_to_string(&first)?;
        let parsed: Value = serde_json::from_str(&first_contents)?;
