...
impl EnvironmentBuilder {
    ...
    pub async fn build(&self) -> Result<EnvironmentContext> {
        ...
        if let Some(cfg) = &self.config {
            ...
            default_headers.extend(cfg.config.default_headers.clone());
            default_headers.extend(profile.config.default_headers.clone());

            let mut env_paths: Vec<PathBuf> = Vec::new();
            if let Some(root_env) = &cfg.config.env {
                push_unique(&mut env_paths, resolve_relative(&self.config_dir, root_env));
            }
            if let Some(profile_env) = &profile.config.env {
                push_unique(
                    &mut env_paths,
                    resolve_relative(&self.config_dir, profile_env),
                );
            }
            if let Some(explicit) = &self.explicit_env {
                push_unique(&mut env_paths, explicit.clone());
            }
...
fn push_unique(envs: &mut Vec<PathBuf>, candidate: PathBuf) {
    if !envs.iter().any(|existing| existing == &candidate) {
        envs.push(candidate);
    }
}
...
mod tests {
    ...
    async fn environment_builder_prefers_explicit_env_and_output() -> Result<()> {
        ...
        write_file(
            ...
            r#"{
  "variables": {"GLOBAL": "global", "ALIAS": "alias"},
  "env": "root.env",
  "defaultHeaders": {
    "X-Root": "root-value",
    "X-Override": "root"
  ...
  "profiles": {
    "dev": {
      "env": "dev.env",
      "variables": {"PROFILE": "dev", "PROFILE_ALIAS": "dev-alias"},
      "responseOutputDir": "profile-responses"
    },
    "staging": {
      "env": "staging.env",
      "variables": {"PROFILE": "staging", "PROFILE_ALIAS": "staging-alias"},
      "responseOutputDir": "staging-responses",
      "defaultHeaders": {
        "X-Staging": "staging-value",
        "X-Override": "staging"
      }
    }
  },
...
"#,
        ...
        assert_eq!(
            environment.initial_env.get("ROOT_ONLY"),
            Some(&"root".to_string())
        );

        assert_eq!(environment.env_files.len(), 3);
        assert!(environment.env_files[0].ends_with(Path::new("root.env")));
        assert!(environment.env_files[1].ends_with(Path::new("staging.env")));
        assert!(environment.env_files[2].ends_with(Path::new("override.env")));

        assert_eq!(
            environment.default_headers.get("X-Root"),
            Some(&"root-value".to_string())
        );
        assert_eq!(
            environment.default_headers.get("X-Staging"),
            Some(&"staging-value".to_string())
        );
        assert_eq!(
            environment.default_headers.get("X-Override"),
            Some(&"staging".to_string())
        );
    ...
    #[tokio::test]
    async fn environment_builder_uses_root_env_when_profile_env_missing() -> Result<()> {
        let temp = tempdir()?;
        let base_dir = temp.path().join("workspace");
        let config_dir = temp.path().join("config");
        std::fs::create_dir_all(&base_dir)?;
...
"#,
        ...
        write_file(&config_dir.join("root.env"), "ROOT_TOKEN=root\n");

        let loaded = load_config(&config_dir)?.expect("config should load");
        let builder = EnvironmentBuilder::new(
            base_dir.clone(),
            config_dir.clone(),
            Some(loaded),
        ...
        let environment = builder.build().await?;
        assert_eq!(environment.profile_name.as_deref(), Some("default"));
        assert_eq!(
            environment.initial_env.get("ROOT_TOKEN"),
            Some(&"root".to_string())
        );
