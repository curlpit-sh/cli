...
const version = env.get("CURLPIT_VERSION") ?? "v0.1.0";
const baseUrl = env.get("CURLPIT_DOWNLOAD_BASE") ?? "https://github.com";

const target = TARGETS[platform]?.[arch];
if (!target) {
  console.error(`Unsupported platform/architecture: ${platform} ${arch}`);
  console.error("Override CURLPIT_PLATFORM/CURLPIT_ARCH or install manually from releases.");
  Deno.exit(1);
}
...
const defaultBinDir = platform === "windows"
...
try {
  ...
  const extractedBinary = await extractArchive(archivePath, tempDir, platform === "windows");
...
async function extractArchive(archivePath: string, temp: string, isWindows: boolean): Promise<string> {
  if (archivePath.endsWith(".tar.gz")) {
    ...
    if (isWindows) {
      ...
      const cmd = new Deno.Command("unzip", { args: [archivePath, "-d", temp] });
      const { success, stderr } = await cmd.output();
      if (!success) {
        throw new Error(new TextDecoder().decode(stderr));
      }
